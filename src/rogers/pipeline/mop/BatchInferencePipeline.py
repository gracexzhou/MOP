...
class BatchInferencePipeline(WLPipeline):
    ...
    def build(self):
        ...
        etl_step = DatabricksStep(name="DB_etl_step",
                                  ...
                                  cluster_log_dbfs_path="dbfs:/FileStore/libs/MOP_cluster/")
    ...
    def publish(self):
        ...
        :return:
        """
        self.published_pipeline = self.pipeline.publish("MOP_Inferencing_Pipeline", "MOP Inferencing Pipeline")

        if self.endpoint is None:
            self.endpoint = PipelineEndpoint.publish(workspace=self.aml_workspace, name=self.config["endpoint_name"],
                                                     description="MOP Inferencing Pipeline",
