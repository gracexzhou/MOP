...
from rogers.pipeline.WLPipeline import WLPipeline


class TrainingPipeline(WLPipeline):
    ...
    def schedule(self, name):
        ...
        Schedule.create(self.aml_workspace, name=name,
                        ...
                        datastore=datastore, path_on_datastore="modem_offline_prediction/mop_test_latest")
    ...
    def publish(self):
        ...
        :return:
        """
        self.published_pipeline = self.pipeline.publish("MOP_Training_Pipeline", "Modem Offline Prediction Training Pipeline")

        if self.endpoint is None:
            self.endpoint = PipelineEndpoint.publish(workspace=self.aml_workspace, name=self.config["endpoint_name"],
                                                     description="MOP Training Pipeline",
