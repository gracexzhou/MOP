...
class ModemCallETL:
    ...
    def __init__(self, modem_accessibility_df: DataFrame, modem_service_quality_df: DataFrame,
                 ...
                 icm_cases_df: DataFrame, resi_df: DataFrame, agg_window: int):
        """
    ...
    def __calls_today(self, df):
        ...
        has_called = self.icm_cases_df.select("account_number", "creation_time"). \
            withColumn(f"call_count", f.count("creation_time").over(call_today_w))
        has_called = has_called.withColumnRenamed("creation_time", "event_date")
        df = df.join(has_called, on=["account_number", "event_date"], how='left').fillna({f'call_count': 0})
        return df
    ...
    def __call_history(self, window: int = 60):
        ...
        def transform(df: DataFrame):
            ...
            df = df.withColumn(f"call_count_{window}", f.sum("call_count").over(call_count_w))
            return df
    ...
    def etl(self):
        ...
        :return:Dataframe
        """
        final_cols = ["account_number", "event_date", "call_count", "call_count_30", "call_count_60", "call_count_10",
                      "call_count_120"]
