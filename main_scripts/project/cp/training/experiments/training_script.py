...
with mlflow.start_run():
    ...
    train_df = Dataset.get_by_name(workspace=ws, name="cp_train_ium").to_pandas_dataframe()
    test_df = Dataset.get_by_name(workspace=ws, name="cp_test_ium").to_pandas_dataframe()
    ...
    for model_name in models:
        ...
        explainer = MimicExplainer(model,
                                   ...
                                   # classes=["no_call", "call"]
                                   )
        explanation = explainer.explain_global(X_test)
        df = explain_sklearn(model, X_train)
        if df is not None:
            df.to_csv(f"{model_name}_explainability_{today}.csv", index=False)
            mlflow.log_artifact(f"{model_name}_explainability_{today}.csv")
        # explain model
